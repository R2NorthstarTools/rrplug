...
struct LogMsg
{
	...
	uint64_t timestamp;
	const char* msg;
	struct MessageSource source;
	int pluginHandle;
};

typedef void (*loggerfunc_t)(struct LogMsg* msg);
...
// Northstar -> Plugin
typedef void (*PLUGIN_INIT_TYPE)(struct PluginInitFuncs* funcs, struct PluginNorthstarData* data);
typedef void (*PLUGIN_INIT_SQVM_TYPE)(struct SquirrelFunctions* funcs);
typedef void (*PLUGIN_INFORM_SQVM_CREATED_TYPE)(enum ScriptContext context, struct CSquirrelVM* sqvm);
typedef void (*PLUGIN_INFORM_SQVM_DESTROYED_TYPE)(enum ScriptContext context);

// Async Communication types

// Northstar -> Plugin
typedef void (*PLUGIN_PUSH_PRESENCE_TYPE)(struct PluginGameStatePresence* data);
typedef void (*PLUGIN_INFORM_DLL_LOAD_TYPE)(enum PluginLoadDLL dll, void* data);
